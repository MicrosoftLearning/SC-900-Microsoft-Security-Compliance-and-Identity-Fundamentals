...
## Demo scenario
In this demo, you will show the capabilities of Microsoft Cloud App Security.  You will walk through the learner through information available on the Cloud Discovery dashboard as well as capabilities available to investigate findings and control impact to your organization through policies.  Note:  An organization must have a license to use Microsoft Cloud App Security which is a user-based subscription service.  
...
1. To see the option for continuous reports, select the **ellipses** on the top-right corner of the page and from the drop-down select **Configure automatic upload**.  There are no data sources connected, but this is where you would add a data source.
...
1. On the Connected apps page, you should see Office 365 on the list with a connected status.
...
1. From the left navigation panel, under where it says Control, select ****Policies**.  The listed policies provide information on the number of alerts generated by the policy.  Selecting any line item provides more detailed information about the policy.  

2. From this policy window on the top right of the page, select ****View open alerts**.  Note how the Alerts option in the left navigation panel is now selected.  This is simply a different way to get to the alerts page.

3. Select an item from the alerts list.  In addition to information about the alert, you can take specific action.  Select the **Resolution options** field to see a list of options.  Select in the space outside of the drop-down box, to close the drop-down box.  

4. From the top-right side of the window, select **Close alert**, to view options for closing the alert.  
