...
In this lab, you'll explore the capabilities of sensitivity labels.  You'll go through the settings for existing sensitivity labels that have been created and the corresponding policy to publish the label.
...
1. From the left navigation pane, expand **Policies** then select **Label publishing policies**.  It is through label policies that sensitivity labels can be published.  The Microsoft 365 tenant has been configured with some label policies, for your convenience. Select **Highly Confidential Policy**.  A window opens that provides information about the policy. Select **Edit policy** from the top of the window.  Here you will walk through the settings without changing anything.
    ...
    1. Default settings for meetings and calendar events: Review the options and note the option to  apply inheritance between Teams meeting and artifacts. Don’t change any settings.  Select **Next**.
...
In this lab, you explored the settings for an existing sensitivity label and the corresponding policy to publish the label.
