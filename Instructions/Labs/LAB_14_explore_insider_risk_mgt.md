...
1. In the window that opens, next to where it says Members check to see if MOD Administrator and Megan Bowen are listed. If they are not listed, then select **Edit** and execute the sub-steps below.  If they are listed, select **Close** and skip the sub-steps below.
...
1. From the Insider risk management overview page, select the **Policies** tab then select **+ Create policy**.  Configure each of the following policy tabs.
    ...
    1. Content to prioritize: Read the description. Leave the default setting - **I want to prioritize content**.  Then under Choose what to prioritize, uncheck everything EXCEPT **Sensitivity labels** (make sure Sensitivity labels remains checked), then select **Next**.
    ...
    1. Triggers: Review the detailed information.  The policy is triggered when a user matches a Data Loss Prevention (DLP) policy or the user performs an exfiltration activity as defined (select the information icons for each bullet point for more detailed information).  Since you don’t have any DLP policy configured as part of this exercise, leave the default setting, **User performs an exfiltration activity**.  Note that policy indicators you enabled in the previous task are checked.  Don't make any change to the selected activities that will trigger the policy.  These indicators will only be activated once the policy is triggered and any activities that map to these indicators  will be used in calculating a risk score for the user. Select **Next**.
    ...
    1. On the page to Decide whether to use fefault or customer indicator thresholds, leave the default setting **Default thresholds**, then select **Next**.
